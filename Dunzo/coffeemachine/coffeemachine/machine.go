...
type BeverageMaker interface {
	...
	AddIngredient(name string, initialQuantity, threshold, bareminimum int)
...
//AddIngredient adds a new type of ingredient in the Coffee Machine, takes name of ingredient, its initial amount and max capacity as parameters
func (cm *CoffeeMachine) AddIngredient(name string, initialQuantity, maxQuantity, bareminimum int) {
...
//MonitorIngredients monitors the current quantity of all the ingredient and shows an error message if its current quantity is less than its bare minimum
func (cm *CoffeeMachine) MonitorIngredients() {
	for {
		for _, v := range cm.ingredients {
			currentQuantity, bareminimum := v.GetQuantity()
			if currentQuantity < bareminimum {
				log.Println(v.GetName(), "quantity is less than bare minimum, please Refill!!")
			}
...
//It will also Monitor the current quantity of all the ingredients
func (cm *CoffeeMachine) Run() {
	for i := 0; i < cm.outlets; i++ {
		go cm.worker()
	}

	go cm.MonitorIngredients()
